...
> Актуализация реализации: первоначальные упоминания PostgreSQL ниже сохранены как часть
> исторического анализа. Текущий backend использует SQLite + `aiosqlite` как единственную
> штатную БД, локальные API-контракты вместо `backend-shared` и не подключает внешний
> `backend-auth`.
